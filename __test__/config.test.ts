...
vi.mock("@savvy-web/github-action-effects", () => {
	// Return a stub — the real module imports @actions/cache which breaks in test
	return { ActionInputs: {} };
});

// ---------------------------------------------------------------------------
// Minimal ActionInputs service for testing (no external deps)
// ---------------------------------------------------------------------------

interface ActionInputsService {
	getOptional: <A, I>(name: string, schema: Schema.Schema<A, I, never>) => Effect.Effect<Option.Option<A>, never>;
}

class ActionInputs extends Context.Tag("ActionInputs")<ActionInputs, ActionInputsService>() {}

const makeInputsLayer = (inputs: Record<string, string>): Layer.Layer<ActionInputs> =>
...
const runDetectBiome = (inputs: Record<string, string>, fsLayer: Layer.Layer<FileSystem.FileSystem>) =>
	Effect.runPromise(
		Effect.provide(
			Effect.flatMap(ActionInputs, (svc) => detectBiome(svc as never)),
			Layer.merge(fsLayer, makeInputsLayer(inputs)),
		),
	);
