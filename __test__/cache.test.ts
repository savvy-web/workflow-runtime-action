...
// ---------------------------------------------------------------------------
// Mock @savvy-web/github-action-effects (same pattern as config.test.ts)
// The real module imports @actions/cache which breaks in test due to minimatch
// ---------------------------------------------------------------------------
...
// Re-import tags after mock so they match what src/cache.ts receives
const { ActionCache } = await import("@savvy-web/github-action-effects");
const { ActionState } = await import("@savvy-web/github-action-effects");
const { ActionEnvironment } = await import("@savvy-web/github-action-effects");
const { CommandRunner } = await import("@savvy-web/github-action-effects");

// ---------------------------------------------------------------------------
// Helpers — cast all layers to Layer.Layer<never> so Effect.provide is happy
// with the mocked GenericTag instances whose types don't align at compile time.
// ---------------------------------------------------------------------------
...
const makeCacheLayer = (opts: {
	restoreResult?: { hit: boolean; matchedKey: string | undefined };
	saveCalls?: CacheSaveCall[];
	restoreCalls?: CacheRestoreCall[];
}): AnyLayer => {
	const saveCalls = opts.saveCalls ?? [];
	const restoreCalls = opts.restoreCalls ?? [];
	const restoreResult = opts.restoreResult ?? { hit: false, matchedKey: undefined };
	...
	return Layer.succeed(ActionCache, {
		...
		restore: (key: string, paths: readonly string[], restoreKeys?: readonly string[]) => {
			restoreCalls.push({ key, paths, restoreKeys });
...
describe("restoreCache", () => {
	...
	it("returns 'exact' when primary key matches and saves state", async () => {
		...
		const dynamicCacheLayer = Layer.succeed(ActionCache, {
			...
			restore: (key: string, _paths: readonly string[], _restoreKeys?: readonly string[]) => {
				return Effect.succeed({ hit: true, matchedKey: key });
			},
			withCache: () => Effect.fail({ _tag: "ActionCacheError", reason: "Not implemented" }),
		} as never);
	...
	it("returns 'partial' when restore key matches", async () => {
		...
		const dynamicCacheLayer = Layer.succeed(ActionCache, {
			save: () => Effect.succeed(undefined),
			restore: () => {
				return Effect.succeed({ hit: true, matchedKey: "some-other-key" });
			},
			withCache: () => Effect.fail({ _tag: "ActionCacheError", reason: "Not implemented" }),
		} as never);
	...
	it("returns 'none' when no cache matches", async () => {
		...
		const layer = Layer.mergeAll(
			makeFileSystemLayer({}),
			makeEnvironmentLayer({ GITHUB_REF: "refs/heads/main" }),
			makeCacheLayer({ restoreResult: { hit: false, matchedKey: undefined } }),
			makeStateLayer({ saved: stateSaved }),
		);
